...
def main(cfg: DictConfig) -> None:
    ...
    checkpoint_path = '/home/ac35anos/PycharmProjects/torch_vct_project_2/outputs/2025-08-14/02-23-41/checkpoints/last.ckpt'

    checkpoint = torch.load(checkpoint_path)
    model_state = checkpoint['state_dict']
    modelmodule.load_state_dict(model_state)
    datamodule.setup()
    val_loader = datamodule.val_dataloader()
    print("Number of clips in val loader:", len(val_loader))


    # inside your eval script / notebook:

    metrics = modelmodule.evaluate_dataset_bpp_psnr(dataloader=val_loader, max_clips=2000)
    print(metrics["avg_psnr"], metrics["avg_bpp"], metrics["avg_bpp_xfirst"])
    # per-clip metrics are in metrics["clips"]
